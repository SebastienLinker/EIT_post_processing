function [ img_data, info_obj ] = createConvex3D( n_images,model,bkgnd,max_size,min_rad,rad_probe,max_pos)
...
for k = 1:1:n_images
    select_fcn = inline(['(x-',num2str(inhomo_positionX(k)),').^2+(y-', ...
        num2str(inhomo_positionY(k)),').^2+(z--3).^2<',num2str(inhomo_size(k)),'^2'],'x','y','z');
...
for k = 1:1:n_images
    select_fcn2 = inline(['(x-',num2str(inhomo_positionX(k)),').^2+(y-', ...
        num2str(inhomo_positionY(k)),').^2+(z--3).^2<',num2str(inhomo_size2(k)),'^2'],'x','y','z');
...
if (nb_to_change > 0)
    [tmp_new_arr, new_info] = createConvex3D(nb_to_change,model,bkgnd,max_size,min_rad,rad_probe,max_pos);
