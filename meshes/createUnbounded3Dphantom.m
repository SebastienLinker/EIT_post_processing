function [ tank, s_tank, n_elements ] = createUnbounded3Dphantom( n_lay, small_mdl )
...
elem_size = [0.5 ref_noROI(t_locref) 100 1000]; %maximal of elements, in mm^3
...
% 	shape_str = [ shape_str ...
% 		'solid farcyl',int2str(k),' = cylinder (',num2str(cog_ref(k,1)),',',num2str(cog_ref(k,2)),...
% 																		',',num2str(t_m_cyl),...
% 					';',num2str(cog_ref(k,1)),',',num2str(cog_ref(k,2)),',',num2str(b_m_cyl),';',...
% 				num2str(max_distance(k+1)),');\n '];% -maxh=',num2str(elem_size(k)),'
% end;
shape_str = [ shape_str ...
    'solid farcyl',int2str(1),' = cylinder(',num2str(cog_ref(1,1)),',',num2str(cog_ref(1,2)),...
    ',',num2str(t_m_cyl),...
    ';',num2str(cog_ref(1,1)),',',num2str(cog_ref(1,2)),',',num2str(b_m_cyl),';',...
    num2str(max_distance(2)),');\n '];
shape_str = [ shape_str ...
    'solid ref_ROI = orthobrick(6,-6.7,0;13,6.7,4);\n '];
...
shape_str = [ shape_str ...
    'solid largecyl',int2str(1),' = (farcyl',int2str(1),' or ref_ROI) and pl1 and pl2 ', ...
    'and not farcyl',int2str(0),'; tlo largecyl',int2str(1),' -maxh=',num2str(elem_size(1)),'; \n '];
for k = 2:1:length(max_distance)-2
    shape_str = [ shape_str ...
        'solid largecyl',int2str(k),' = farcyl',int2str(k),' and pl1 and pl2 ', ...
        'and not farcyl',int2str(k-1),' and not ref_ROI; tlo largecyl',int2str(k),' -maxh=',num2str(elem_size(k)),'; \n '];
end

k = length(max_distance)-1;
shape_str_large = [shape_str ...
    'solid largecyl',int2str(k),' = farcyl',int2str(k),' and pl1 and pl2 ', ...
    'and not farcyl',int2str(k-1),' and not ref_ROI; tlo largecyl',int2str(k),'; \n ',...
...
if debug
    tank = ng_mk_gen_models( shape_str_large, elec_pos, elec_shape, elec_obj )
    ...
    [~, tank] = evalc('ng_mk_gen_models( shape_str_large, elec_pos, elec_shape, elec_obj )');
